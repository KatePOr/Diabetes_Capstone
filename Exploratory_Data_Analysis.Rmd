...
All graphs were used to further understand the data. Graphs used in the report were labeled as **Used in report**.
...
### _AGE80 summary (Age between 18 and 80 years old) - KO  **Used in report**
```{r}
# Density Plot of _AGE80 separated by type 2 diabetes
CDC_2023_subset %>%
  # Rename variables to simplify axes and legends
  mutate(DIABETE4 = str_replace(DIABETE4, "0", "No Diabetes")) %>%
  mutate(DIABETE4 = str_replace(DIABETE4, "1", "Type 2 Diabetes")) %>%
  ggplot(aes(x = AGE80, fill = DIABETE4)) + 
...
# Density Plot of BMI separated by type 2 diabetes
CDC_2023_subset %>%
  # Rename variables to simplify axes and legends
  mutate(DIABETE4 = str_replace(DIABETE4, "0", "No Diabetes")) %>%
  mutate(DIABETE4 = str_replace(DIABETE4, "1", "Type 2 Diabetes")) %>%
  ggplot(aes(x = BMI, fill = DIABETE4)) + 
    geom_density(alpha=0.4) + 
    scale_y_continuous(labels = scales::comma)+
    ylab("Density") +
    xlab("BMI") + 
...
## Compare Diabetes and Exercise to Determine the Impact on Type 2 Diabetes - SG  **Used in report**
```{r}
...
## Measures of central tendencies, spread, and measures of distribution shape (skew and kurtosis) and histogram for all and type 2 diabetes within BMI - KO **Used in report**
```{r}
...
## Measures of central tendencies, spread, and measures of distribution shape (skew and kurtosis) and histogram for all and type 2 diabetes within _AGE80 - KO **Used in report**
```{r}
# Summary of BMI column
print(summary(CDC_2023_subset$AGE80))
cat("Skewness is ", skewness(CDC_2023_subset$AGE80), ". ")
cat("Kurtosis is ", kurtosis(CDC_2023_subset$AGE80), ".")

# Histogram 
AGE_summ1 <- CDC_2023_subset %>% summarize(mean = mean(AGE80), median = median(AGE80)) #Calculate mean for graph
ggplot(data = CDC_2023_subset, aes(x = AGE80)) +
  geom_histogram(aes(y= ..density..), binwidth = 0.5, fill="grey") +
  geom_density(linewidth = 0.7) +
  geom_vline(aes(xintercept = mean), AGE_summ1, color = "red", linewidth = 0.7) +
  geom_vline(aes(xintercept = median), AGE_summ1, color = "blue", linewidth = 0.7) + 
  xlab("BMI") +
  ylab("Density") + 
...
### Proportion tables for each of the categories (Social Factors, Demographics, and Diseases) split by type 2 diabetes - KO **Used in report**
```{r}
# Social factors
CDC_2023_subset %>%
  # Rename variables to simplify axes and legends
...
CDC_2023_subset %>%
  ...
  # Create proportion table
  tbl_summary(
    include = c(Education, Employment_Status, Income, Sex, BMI, Age),
    by = Type_2_Diabetes,
    missing = 'no',
    type = all_dichotomous() ~ "categorical"
  ) |>
...
### Compare Education with Type_2_Diabetes using bar plots (stacked bar for overall numbers and fill bar plots for proportions) - KO **Used in report**
```{r}
# Stacked Bar Plot
CDC_2023_subset %>%
  # Rename variables to simplify axes and legends
...
subset <- CDC_2023_subset %>%
  select(Education, Employment_Status, Income, Sex, BMI, Age) 
# Create heat map
model.matrix(~0+., data=subset) %>% 
  cor(use="pairwise.complete.obs") %>% 
  ggcorrplot(show.diag=FALSE, type="lower", lab=TRUE, lab_size=2)
...
### Compare Employment_Status, Education, and Type 2 Diabetes - KO **Used in report**
```{r}
...
### Compare Mental_Health, Education, and Type 2 Diabetes - KO
```{r}
df_summary <- CDC_2023_subset %>%                                                                 
...
### Compare Not_Afford_Doctor, Education, and Type 2 Diabetes - KO
```{r}
df_summary <- CDC_2023_subset %>%                                                                 
...
ggplot(df_summary, aes(x = Education, y = Not_Afford_Doctor, size=Count, color=Count)) +             
  ...
  facet_wrap(~Type_2_Diabetes) +
  coord_flip() + 
  ylab("Can the individual afford a doctor?") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
