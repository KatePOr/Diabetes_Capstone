...
```{r}
CDC_2023_subset = subset(CDC_2023_subset, select = -c(1, 5:6, 24) )
```
...
```{r}
# Exclude columns 20 and 21 (assuming these are AGE and BMI)
col_names <- col_names[-c(20, 21)]
...
### DRNK3GE5 summary (0: No binge drinking in last 30 days, 1: yes, binge drinking in last 30 days) - KO
```{r}
# Proportion summary
...
# Calculate proportions for education levels by diabetes status & plot (precleaned data) - SG
```{r}
# Calculate proportion
education_diabetes_proportion <- CDC_2023 %>%
  group_by(EDUCA, DIABETE4) %>%  # Group by education level and diabetes status
  summarise(count = n(), .groups = 'drop') %>%  # Count occurrences
  group_by(EDUCA) %>%  # Group by education level
  mutate(proportion = count / sum(count))  # Calculate proportions

# Print table to view proportions
print(education_diabetes_proportion)

# Create bar plot to display proportions
ggplot(education_diabetes_proportion, aes(x = EDUCA, y = proportion, fill = as.factor(DIABETE4))) +
  geom_bar(stat = "identity", position = "fill") +  # Stacked bar chart normalized to show proportions
  labs(
    title = "Education Level Distribution by Type 2 Diabetes Status",
    x = "Education Level",
    y = "Proportion",
...
# Proportion table and bar graph comparing Kidney Disease and Type 2 Diabetes - SG
```{r}
# Create a proportion table for Kidney Disease (CHCKDNY2) and Type 2 Diabetes (DIABETE4)
proportion_table <- CDC_2023_subset %>%
  group_by(CHCKDNY2, DIABETE4) %>%
  summarise(Count = n()) %>%
  mutate(Proportion = Count / sum(Count))

# Rename levels of Kidney Disease and Diabetes for better readability
proportion_table$CHCKDNY2 <- factor(proportion_table$CHCKDNY2, 
                                    levels = c(1, 2),
                                    labels = c("Yes", "No"))

proportion_table$DIABETE4 <- factor(proportion_table$DIABETE4, 
                                    levels = c(1, 0),
                                    labels = c("Type 2 Diabetes", "No Diabetes"))

# Create a bar graph to visualize the correlation
ggplot(proportion_table, aes(x = CHCKDNY2, y = Proportion, fill = DIABETE4)) +
  geom_bar(stat = "identity", position = "fill") +
  labs(
    title = "Proportion of Kidney Disease by Type 2 Diabetes Status",
    x = "Kidney Disease",
    y = "Proportion",
    fill = "Diabetes Status"
  ) +
...
### Comapre Employment_Status, Education, and Sex - KO
```{r}
...
### Comapre Income, Education, and Sex - KO
```{r}
df_summary <- CDC_2023_subset %>%                                                                 
...
### Comapre Mental_Health, Education, and Sex - KO
```{r}
df_summary <- CDC_2023_subset %>%                                                                 
...
### Comapre Not_Afford_Doctor, Education, and Sex - KO
```{r}
df_summary <- CDC_2023_subset %>%                                                                 
...
### Compare Physical_Health, Education, and Sex - KO
```{r}
df_summary <- CDC_2023_subset %>%                                                                 
